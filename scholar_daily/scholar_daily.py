...
Here is the list {list(self.All_result_dict.keys())}"""}
    ...
    def get_News(self):
        ...
        bot_id = "7446278637571670025"
        user_id = "ScholarDaily"
        full_message_content = ""
        for event in coze.chat.stream(
            bot_id=bot_id, user_id=user_id, additional_messages=[Message.build_user_question_text("告诉我最新的10条ai新闻")]
        ):
            if event.event == ChatEventType.CONVERSATION_MESSAGE_DELTA:
                message = event.message
                if message and message.content:
                    full_message_content += message.content
        self.News = full_message_content
    ##############################
